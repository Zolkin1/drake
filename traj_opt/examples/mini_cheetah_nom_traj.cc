...
namespace traj_opt {
namespace examples {
namespace mini_cheetah_nom_traj {
...
public:
    ...
    void RunExample(const std::string options_file) const {
        ...
        // TODO: Make sure the correct thing is being visualized
    ...
    void RunModelPredictiveControl(
        ...
        // Contact visualizer
        drake::multibody::meshcat::ContactVisualizerParams contact_viz_params{
                1/32.0,
                ...
                3,
                0.002 };
        auto& contact_viz = drake::multibody::meshcat::ContactVisualizer<double>::AddToBuilder(&builder, plant, meshcat_, contact_viz_params);
        std::cout << "contaact viz's input port: " << contact_viz.contact_results_input_port().get_name() << std::endl;
        ...
        // TODO: Need to shift the RoM trajectory through the MPC cost
    ...
    void SetProblemDefinition(const TrajOptExampleParams& options,
        ...
        // Create the RoM trajectory
        std::cout << "Generating trajectory" << std::endl;
        // TODO: make this take in cost function and/or desired location

        // Create a diff drive
        auto diff_drive = std::make_unique<DiffDrive<double>>(options.diff_drive_target,  options.diff_drive_horizon);
        diff_drive->SetPMatrix(options.diff_drive_P);
        diff_drive->SetRMatrix(options.diff_drive_R);
        diff_drive->SetQMatrix(options.diff_drive_Q);
        diff_drive->SetVelocitySaturation(options.diff_drive_velocity_saturation);
        diff_drive->SetAngularSaturation(options.diff_drive_angular_saturation);
        opt_prob->q_nom = diff_drive->GenerateXYZTrajectory(opt_prob->q_init, opt_prob->num_steps, options.time_step);
        diff_drive->DisplayTrajectory(meshcat_);
        std::cout << "Trajectory generated." << std::endl;

        // TODO: plot this nominal trajectory

        std::cout << opt_prob->q_nom.size() << std::endl;
        std::cout << opt_prob->num_steps << std::endl;
        std::cout << "now assigning v_nom" << std::endl;
    ...
    void PlayBackTrajectory(const std::vector<VectorXd>& q,
        ...
        // Contact visualizer
        auto& contact_viz = drake::multibody::meshcat::ContactVisualizer<double>::AddToBuilder(&builder, plant, meshcat_);
...
int main(int argc, char* argv[]) {
    ...
    drake::traj_opt::examples::mini_cheetah_nom_traj::TrajOptGenerator example;
